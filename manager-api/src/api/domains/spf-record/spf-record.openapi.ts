  ...
  applyDecorators(
    ApiOperation({
      ...
      description:
        ...
        "name of this server resolves to) may send for it. It always starts with `mx` and the `ip4:` term of each of those addresses; " +
        "from a published record, only the third-party senders (`include`, `a`, `mx` naming another host, `exists`) and its ending " +
        "(`all` or `redirect=`, `-all` when it has none) are kept, and any other address is left out. `published` is the " +
