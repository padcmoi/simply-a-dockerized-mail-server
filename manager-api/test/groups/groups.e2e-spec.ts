...
// The HTTP verbs the route tables dispatch on; each maps to a real supertest
// agent method, so `call` needs no index-signature cast.
...
describe("GroupsController (e2e: auth + ACL + behavior)", () => {
  ...
  const call = (method: HttpMethod, path: string): request.Test => {
    const agent = api();
    if (method === "get") return agent.get(path);
    if (method === "post") return agent.post(path);
    if (method === "put") return agent.put(path);
  ...
  const SERVICE_ROUTES: Array<{ m: HttpMethod; p: string; svcKey: keyof typeof svc; body?: object }> = [
