...
describe("HealthcheckService", () => {
  let ds: ReturnType<typeof providerMock<DataSource>>;
  ...
  beforeEach(() => {
    ...
    ds = providerMock<DataSource>({ query: vi.fn().mockResolvedValue([{ ok: 1 }]) });
    svc = new HealthcheckService(ds);
  });

  // The private numeric probes snapshot() composes. The tier/status branches are
  // deterministic only once their outputs are pinned (and stubbing cpu() also
  // skips the real 100ms sampler). The intersection widens `svc` to expose them
  // for spying while keeping each stubbed return value fully type-checked.
  type Probes = { cpu(): Promise<number>; memory(): Promise<number>; loadScore(): number };
  const probes = (s: HealthcheckService): HealthcheckService & Probes => s as HealthcheckService & Probes;

  // Force the numeric signals so the tier/status branches are deterministic.
  const stub = (cpu: number, memory: number, load: number) => {
    vi.spyOn(probes(svc), "cpu").mockResolvedValue(cpu);
    vi.spyOn(probes(svc), "memory").mockResolvedValue(memory);
    vi.spyOn(probes(svc), "loadScore").mockReturnValue(load);
  };
  ...
  it("reports 0% memory when total memory reads as zero (defensive guard)", async () => {
    ...
    vi.spyOn(probes(svc), "cpu").mockResolvedValue(10);
    vi.spyOn(probes(svc), "loadScore").mockReturnValue(10);
    state.os.totalmem = 0;
    const snap = await svc.snapshot();
  ...
  it("treats a missing load-average sample as zero load", async () => {
    vi.spyOn(probes(svc), "cpu").mockResolvedValue(10);
    vi.spyOn(probes(svc), "memory").mockResolvedValue(10);
    state.os.loadavg = [];
    const snap = await svc.snapshot();
