...
import { SieveRejectSender } from "../../src/core/entities/sieve-reject-sender.entity";
import { repoMock } from "../helpers/mocks";

describe("RejectSendersService", () => {
  let repo: ReturnType<typeof repoMock<SieveRejectSender>>;
  let svc: RejectSendersService;

  beforeEach(() => {
    repo = repoMock<SieveRejectSender>();
    // save echoes a persisted row (id assigned) -- non-default, kept as before.
    repo.save.mockImplementation((x) => Promise.resolve({ id: 1, ...x }));
    svc = new RejectSendersService(repo);
  });
  ...
  describe("list", () => {
    ...
    it("paginates, searches and sorts when limit is present", async () => {
      ...
      const res = await svc.list({ limit: 10, offset: 0, search: "x", sortBy: "sender", sortDir: "asc" });
    ...
    it("falls back to the default sort column on an unknown sortBy", async () => {
      ...
      await svc.list({ limit: 10, offset: 0, sortBy: "bogus", sortDir: "desc" });
