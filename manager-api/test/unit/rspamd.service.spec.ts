...
const okJson = (data: unknown): Response => ({ ok: true, status: 200, json: async () => data }) as unknown as Response;
const notOk = (status: number): Response => ({ ok: false, status, json: async () => ({}) }) as unknown as Response;
...
describe("RspamdService", () => {
  ...
  describe("history", () => {
    ...
    it("requests the explicit size and filters to the domain's recipients", async () => {
      fetchMock.mockResolvedValue(okJson({ rows: rows() }));
      const res = (await svc.history("example.com", 50)) as RspamdHistoryRow[];
    ...
    it("tolerates rows without a rcpt_smtp array while filtering", async () => {
      ...
      const res = (await svc.history("example.com", 200)) as RspamdHistoryRow[];
      expect(res).toEqual([]);
    });
    ...
    describe("paginated", () => {
      ...
      const paginate = (q: Record<string, unknown>) =>
        svc.history(undefined, 200, { offset: 0, sortDir: "desc", ...q }) as Promise<{
          items: RspamdHistoryRow[];
          total: number;
        }>;
