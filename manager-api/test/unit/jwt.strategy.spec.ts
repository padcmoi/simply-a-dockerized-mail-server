...
describe("JwtStrategy", () => {
  ...
  describe("validate", () => {
    ...
    it("defaults isRoot to false for a non-true value", async () => {
      ...
      // any non-boolean-true (e.g. an old token) is also treated as non-root
      const coerced = await strategy.validate({ sub: "u1", email: "e@x.com", isRoot: "yes" as never });
      expect(coerced.isRoot).toBe(false);
    });

    it("rejects a payload with no subject", async () => {
      await expect(strategy.validate({ email: "e@x.com" } as never)).rejects.toBeInstanceOf(UnauthorizedException);
      await expect(strategy.validate(null as never)).rejects.toBeInstanceOf(UnauthorizedException);
