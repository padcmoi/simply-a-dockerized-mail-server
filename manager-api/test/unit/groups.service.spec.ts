...
import type { ActingUser } from "../../src/core/acl/anti-escalation.service";
...
// A chainable query-builder double: every fluent method returns the same object,
// clone() included, so listMembers' base/rowsQb split resolves to one recorder
// whose orderBy/limit/offset/andWhere calls (and getRawMany/getCount results) the
// test can assert.
function makeQb() {
  const qb: Record<string, ReturnType<typeof vi.fn>> = {};
  for (const method of ["innerJoin", "leftJoin", "where", "andWhere", "clone", "select", "addSelect", "orderBy", "limit", "offset"]) {
    qb[method] = vi.fn(() => qb);
  }
  qb.getRawMany = vi.fn();
  qb.getCount = vi.fn();
...
function makeMocks() {
  ...
  const profiles = { find: vi.fn() };
  const groupMembers = { find: vi.fn(), findOne: vi.fn(), count: vi.fn() };
  const domains = { findBy: vi.fn() };
  const cpg = {
    guard: {
      createGroup: vi.fn(),
      setGroupOwner: vi.fn(),
      ...
      utils: { check: { global: vi.fn() } },
    },
    rawDeleteGroup: vi.fn(),
    rawSetGroupGlobalPermissions: vi.fn(),
  };
  const auditLog = { record: vi.fn() };
  const antiEscalation = { assertActingUserHolds: vi.fn(), assertActingUserCanReplace: vi.fn() };
  return { qb, groups, accounts, profiles, groupMembers, domains, cpg, auditLog, antiEscalation };
}

const q = (over: Partial<PaginationQuery> = {}): PaginationQuery =>
  ({ offset: 0, sortDir: "desc", ...over }) as PaginationQuery;
...
describe("GroupsService", () => {
  ...
  describe("setGlobalPermissions", () => {
    const perms = [{ resource: "sieve", action: "access" }];

    it("a non-root goes through the lockout-protected lib path and audits", async () => {
      m.groups.findOne.mockResolvedValue(makeGroup());
      m.cpg.guard.findGroupGlobalPermissions.mockResolvedValue([]);
      await svc.setGlobalPermissions(GID, USER, perms as never);
    ...
    it("root uses the raw escape hatch, bypassing anti-lockout", async () => {
      m.groups.findOne.mockResolvedValue(makeGroup());
      await svc.setGlobalPermissions(GID, ROOT, perms as never);
    ...
    it("propagates an anti-escalation refusal before writing", async () => {
      ...
      await expect(svc.setGlobalPermissions(GID, USER, perms as never)).rejects.toBeInstanceOf(ForbiddenException);
      expect(m.cpg.guard.setGroupGlobalPermissions).not.toHaveBeenCalled();
    });
  });

  describe("setDomainPermissions", () => {
    const dperms = [{ domainId: 1, resource: "recipients", action: "access" }];
    ...
    it("validates that referenced domains exist, then writes and audits", async () => {
      ...
      await svc.setDomainPermissions(GID, USER, dperms as never);
    ...
    it("rejects with 404 when a referenced domain does not exist", async () => {
      ...
      await expect(svc.setDomainPermissions(GID, USER, dperms as never)).rejects.toBeInstanceOf(NotFoundException);
    ...
    it("skips the domain existence check for an empty permission set", async () => {
      m.groups.findOne.mockResolvedValue(makeGroup());
      await svc.setDomainPermissions(GID, USER, [] as never);
    ...
    it("propagates an anti-escalation refusal", async () => {
      ...
      m.domains.findBy.mockResolvedValueOnce([{ id: 1, domain: "d1" }]);
      m.antiEscalation.assertActingUserCanReplace.mockRejectedValueOnce(new ForbiddenException("nope"));
      await expect(svc.setDomainPermissions(GID, USER, dperms as never)).rejects.toBeInstanceOf(ForbiddenException);
      expect(m.cpg.guard.setGroupDomainPermissions).not.toHaveBeenCalled();
    });
  });
