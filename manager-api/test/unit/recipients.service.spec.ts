...
import type { CreateRecipientDto, UpdateRecipientDto } from "../../src/api/domains/recipients/recipients.validation";
...
describe("RecipientsService", () => {
  ...
  let domains: { findOne: ReturnType<typeof vi.fn> };
  let recipientQuotas: { find: ReturnType<typeof vi.fn>; findOne: ReturnType<typeof vi.fn>; delete: ReturnType<typeof vi.fn> };
  let storage: { removeRecipient: ReturnType<typeof vi.fn> };
  ...
  beforeEach(() => {
    ...
    domains = { findOne: vi.fn() };
    recipientQuotas = { find: vi.fn(), findOne: vi.fn(), delete: vi.fn() };
    storage = { removeRecipient: vi.fn() };
    svc = new RecipientsService(recipients as never, domains as never, recipientQuotas as never, storage as never);
  });
  ...
  describe("list (legacy, no limit)", () => {
    it("returns rows ordered by email with usedBytes attached (matched + back-filled '0')", async () => {
      ...
      const res = await svc.list(FQDN, {} as never);
  ...
  describe("list (paginated)", () => {
    it("searches, sorts on the joined usedBytes column, and maps raw usedBytes (missing -> '0')", async () => {
      ...
      const res = await svc.list(FQDN, { limit: 10, offset: 0, search: "jd", sortBy: "usedBytes", sortDir: "asc" } as never);
    ...
    it("without a search term, falls back to the id column on an unknown sortBy and orders on r.<col>", async () => {
      ...
      await svc.list(FQDN, { limit: 25, offset: 25, sortBy: "bogus", sortDir: "desc" } as never);
  ...
  describe("create", () => {
    const dto = (over: Partial<CreateRecipientDto> = {}): CreateRecipientDto =>
      ({ localPart: "jdoe", password: "correcthorse", quota: 104857600, ...over }) as CreateRecipientDto;
  ...
  describe("update", () => {
    ...
    it("403s (postmasterImmutable) for postmaster@<domain>", async () => {
      ...
      const e = await rejection(svc.update(5, {} as UpdateRecipientDto, FQDN));
    ...
    it("400s (quotaBelowUsage) when the new quota is under what the mailbox already stores", async () => {
      ...
      const e = await rejection(svc.update(5, { quota: 1048576 } as UpdateRecipientDto, FQDN));
      expect((e as ApiError).getStatus()).toBe(400);
    ...
    it("400s (quotaExceedsDomain) when a raised quota does not fit the domain", async () => {
      ...
      const e = await rejection(svc.update(5, { quota: 104857600 } as UpdateRecipientDto, FQDN));
      expect((e as ApiError).getStatus()).toBe(400);
      expect((e as ApiError).getResponse()).toMatchObject({ code: "recipients.quotaExceedsDomain" });
    });
    ...
    it("shrinking a quota bypasses the domain-fit check (headroom never consulted); a mailbox with no quota row yet reads as 0 used", async () => {
      ...
      await svc.update(5, { quota: 52428800, active: true } as UpdateRecipientDto, FQDN);
    ...
    it("applies password, a raised (fitting) quota, active:false and end-date, then saves", async () => {
      ...
      await svc.update(
        5,
        { password: "newpass", quota: 104857600, active: false, userEndDate: "2027-06-01" } as UpdateRecipientDto,
        FQDN
      );
    ...
    it("with no quota in the body, never touches the usage/domain checks", async () => {
      ...
      await svc.update(5, { active: false } as UpdateRecipientDto, FQDN);
