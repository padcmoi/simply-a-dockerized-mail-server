...
describe("DkimService (OpenDKIM sidecar over global fetch)", () => {
  ...
  beforeEach(() => {
    fetchMock.mockReset();
    repo = { find: vi.fn(), upsert: vi.fn().mockResolvedValue(undefined), delete: vi.fn().mockResolvedValue(undefined) };
    const cfg = { get: vi.fn().mockReturnValue(BASE) };
    svc = new DkimService(cfg as never, repo as never);
  });
  ...
  describe("constructor", () => {
    it("falls back to the default sidecar URL when OPENDKIM_API_URL is unset", async () => {
      ...
      const local = new DkimService(cfg as never, repo as never);
