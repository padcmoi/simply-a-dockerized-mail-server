...
function makeRepo() {
  return {
    create: vi.fn((x: Partial<ApiToken>) => x),
    save: vi.fn((x: Partial<ApiToken>) => Promise.resolve({ id: 1, createdAt: new Date("2020-01-01T00:00:00.000Z"), ...x })),
    find: vi.fn(),
    findOne: vi.fn(),
    delete: vi.fn(),
  };
...
function tokenFor(secret: string, over: Partial<ApiToken> = {}): ApiToken {
  return {
    ...
    account: { id: "acc-1", email: "a@b.com", isRoot: 0, enabled: 1 },
    ...over,
  } as ApiToken;
}
...
describe("ApiTokenService", () => {
  ...
  beforeEach(() => {
    repo = makeRepo();
    svc = new ApiTokenService(repo as never);
  });
  ...
  describe("validate", () => {
    ...
    it("rejects a token on a disabled account", async () => {
      repo.findOne.mockResolvedValueOnce(tokenFor("s3cret", { account: { id: "acc-1", email: "a@b.com", isRoot: 0, enabled: 0 } as never }));
      expect(await svc.validate("sms_cid.s3cret", "1.2.3.4")).toBeNull();
    });
    it("accepts a valid key, normalises the IP through the allow-list, and stamps last use", async () => {
      const row = tokenFor("s3cret", {
        ...
        account: { id: "acc-1", email: "a@b.com", isRoot: 1, enabled: 1 } as never,
        failedAttempts: 2,
      });
      repo.findOne.mockResolvedValueOnce(row);
