...
// the paths handed to readdir/stat verifies the spool layout it builds.
...
function makeService(spoolPath?: string): PostfixService {
  return new PostfixService({ get: () => spoolPath } as never);
}
...
describe("PostfixService.queueStats", () => {
  ...
  it("counts per-domain matches by scanning message bodies, skipping unreadable files", async () => {
    mockReaddir.mockImplementation(async (p: string) => (p === "/spool/active" ? ["m1", "m2", "bad"] : []));
  ...
  it("falls back to the default spool path when the config has none", async () => {
    ...
    expect(mockReaddir).toHaveBeenCalledWith("/var/spool/postfix/active");
    expect(mockReaddir).toHaveBeenCalledWith("/var/spool/postfix/incoming");
  });
});
