...
describe("QuotasController (e2e: auth + ACL + behavior)", () => {
  ...
  const domainRepo = repoMock<VirtualDomain>();
  domainRepo.findOne.mockImplementation(async ({ where }: { where: { id: number } }) => ({
    id: where.id,
    domain: FQDN,
    ownerId: null,
    quota: DOMAIN_QUOTA,
  }));
  const domainQuotasRepo = repoMock<VirtualQuotaDomain>();
  domainQuotasRepo.findOne.mockResolvedValue(null);
