#!/usr/bin/env bash
# Postfix attachment cap and dovecot quota tracking via VirtualQuotaUsers.
...
t_quota_updates() {
  local before after
  before=$(docker exec mail-mariadb mariadb -uroot -p"$DB_ROOT_PASSWORD" -BN mailserver -e "SELECT COALESCE(bytes,0) FROM VirtualQuotaUsers WHERE email='$(email_of at2)'" 2>/dev/null)
  inject_mail at2 INBOX "quota-probe@example.com" "quota-$$" "" "$(printf 'A%.0s' {1..1024})"
  sleep 2
  after=$(docker exec mail-mariadb mariadb -uroot -p"$DB_ROOT_PASSWORD" -BN mailserver -e "SELECT COALESCE(bytes,0) FROM VirtualQuotaUsers WHERE email='$(email_of at2)'" 2>/dev/null)
