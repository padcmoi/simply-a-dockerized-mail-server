...
- Every service runs on a pinned image (no Debian as build base). Choices :
  ...
  - clamav : `clamav/clamav:1.4.4` (official, opt-in profile)
  - phpmyadmin : `phpmyadmin:5.2` (official, opt-in profile, exposed on `:8082` for DB inspection)
...
- Root account credentials never persisted on disk : `install.sh` generates email + password in memory only, inserts via the `99_manager_api_auth.sql` bootstrap (run once on fresh MariaDB datadir), and prints them ONCE to TTY at the end of install. The DB (existing `Accounts` table, additive columns) is the single source of truth, queried by manager-api at login. Bootstrap is skipped on re-installs once MariaDB datadir is initialized.
...
- Login consolidated on the existing 1.x.x `Accounts` table : additive columns `password_hash` (SHA512-CRYPT), `role`, `is_active`, `created_at`, `last_login_at`. The standalone `webadmin_accounts` table + its NestJS module have been removed (FK targets stay valid, existing 1.x.x rows with `password_hash IS NULL` simply cannot log in).
- Refresh JWTs now persisted in `RefreshTokens` (jti + account_id + expires_at + revoked_at + user_agent + ip). Refresh rotates : the consumed row is marked revoked and a fresh jti is issued. `POST /api/v1/auth/logout` revokes one token, `POST /api/v1/auth/logout-all` revokes every active session for the current principal, no more secret rotation needed for surgical revocation.
- `install.sh` hardened : ERR trap surfaces the failing line + command + exit code (no more silent `set -e` aborts) ; secret generation no longer uses `tr </dev/urandom | head -c N` (which races with `set -o pipefail` via SIGPIPE) - replaced by `openssl rand -base64` into a bash variable + slicing ; `root_jwt_secret` generation moved before the email prompt block so the API container always has its secret to mount, even if the bootstrap is skipped.
- OpenAPI decoration extracted out of every controller into its sibling `<feature>.openapi.ts` file : each endpoint imports one `@OpenApi<Verb>()` factory (built via `applyDecorators(...)`) that bundles `ApiOperation` + `ApiOkResponse` + the relevant `ApiBearerAuth` / `ApiQuery` / `ApiUnauthorizedResponse`. Controllers keep only `@ApiTags` at class level, mirror of the `docker-nestjs-template` `api.controller.ts` + `api.openapi.ts` pattern.
- Swagger UI moved from `/api/v1/docs` to `/api/doc` (versionless URL), JSON spec at `/api/doc-json`. `persistAuthorization: true` so the bearer token survives page reloads. Setup code lives in `manager-api/src/openapi.ts`, `main.ts` just calls `setupOpenApi(app)`.
