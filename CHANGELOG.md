...
- install.sh also installs a certbot deploy hook at `/etc/letsencrypt/renewal-hooks/deploy/simply-mailserver`. On every successful renewal of the stack's TLS cert (filtered by `TLS_CERT_NAME` from .env so other certs on the same host stay untouched), the hook runs `docker compose restart dovecot postfix` so they pick up the new fullchain/privkey. Logs to journald under `letsencrypt-hook[simply-mailserver]`
