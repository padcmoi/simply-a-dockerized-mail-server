...
- chore(ci): run the test suite as three stages instead of one long chain: the three manager-api jobs (lint, typecheck, test) run in parallel, then the three manager-ui jobs run in parallel, then `test-mailservers.sh`. Each job is self-contained (its own checkout/install/run), so the intra-stage jobs have no reason to serialise; wall-clock drops to the slowest job per stage while a broken manager-api still stops the run before the heavier UI and mail-stack stages (13-07-2026)
- fix(test): the manager-ui `test`/`test:cov` scripts run `nuxt prepare` before vitest, so a clean checkout (the CI `manager-ui · test` job) has the generated `.nuxt/tsconfig.json` that `tsconfig.json` extends. Without it vite failed to resolve the tsconfig and every spec errored; it only passed locally because the earlier typecheck/lint steps had already generated `.nuxt`. Mirrors the existing lint/typecheck scripts, which already prepend `nuxt prepare` (13-07-2026)
- test(manager): add a vitest unit + integration + e2e suite to manager-api and manager-ui, run by `test-node.sh` (the `.husky/pre-push` gate) and by dedicated GitHub Actions jobs (`manager-api · test` with coverage, `manager-ui · test`). manager-api (vitest via unplugin-swc so NestJS DI/decorators resolve as at runtime, no database/containers/network): a security cartography reflects every route off its controller metadata and locks the full auth + ACL contract in a snapshot with invariants (global auth guard wired, the exact allowlist of public routes, every authenticated route carries an ACL unless self-scoped/root-enforced, domain routes require both guards and a `:domainId`, permissions valid against the catalog, all controllers registered); one e2e spec per controller asserts 401/403/2xx auth + ACL through a real-guards harness with mocked services; one unit spec per service/guard/util covers the branches, all through mocked repositories. Coverage is 99.88% statements / 99.21% branches with a 95% threshold enforced (branches 90%). manager-ui (vitest, pure logic only, no component rendering or Playwright) covers composables, Pinia stores and utils. `test-node.sh` now runs typecheck + lint + test for both apps (13-07-2026)
- fix(ci): seed the CI mail-server admin account against the email-identity schema. The seed read `MANAGER_ADMIN_USERNAME`/`MANAGER_ADMIN_PASSWORD` from `.env` (both removed from `.env.sample` by the email refactor), so a `grep` under `set -e` aborted the step, and `ci/seed.sql` still inserted the dropped `accounts.username` column. CI now mints its own `admin@<primary_domain>` and a random password, seeds the account by email with its 1-1 `account_profiles` row (mirroring install.sh), and the schema wait polls for `account_profiles` (created just before the migration that finalises the accounts schema) instead of `accounts` (which exists earlier, still carrying `username`) (13-07-2026)
- feat(rspamd): a domain's Rspamd page shows a standalone `Bayes learning` card next to the donut stats card in an equal-height two-column grid (both cards stretch to the same height and the pager is pinned to the card's bottom, matching the domain dashboard cards), listing the classifier's learn counts per recipient (ham in green, spam in red) with the domain totals in the header, the recipients ranked by total learns and paginated 4 per page. rspamd's own `/stat` endpoint only exposes a server-wide aggregate, so a minimal RESP reader on the existing `mail-redis` socket does `SMEMBERS` on the Bayes key sets and `HMGET` per statfile (the classifier is `per_user` with an `rcpt:addr` selector, so each key carries the recipient), no new client dependency. `GET /domains/:id/rspamd/stats` returns a `bayes` block that merges every mailbox of the domain (`virtual_users`, 0 learns when untrained) with the Redis counts, so the card shows the full ranked recipient set rather than only trained addresses; an unreachable Redis degrades to an empty block, never a 5xx. The server-wide `/rspamd` page is unchanged, and the card is gated by the same `rspamd` `access` + `view-rspamd-stats` permissions as the stats endpoint (13-07-2026)
- chore(install): install.sh asks for the root account's email and password at seed time and keeps them only in memory. Neither `MANAGER_ADMIN_EMAIL` nor `MANAGER_ADMIN_PASSWORD` is ever written to `.env` (both removed from `.env.sample`), and the summary banner never prints a chosen password (only an auto-generated one, once, to the terminal). The account is seeded by email with its 1-1 profile row (12-07-2026)
- feat(ui): login is by email; the sidebar shows the account with `UUser` (display name + email, a root chip, truncating text, avatar-only when collapsed). The profile page edits the full profile: email, display name, phone, avatar (via a modal with a live preview), address, address complement, city, postal code, country as a searchable select of every country with a flag emoji and a clear button, and read-only geocoded coordinates shown as text. Accounts, group members/owner, domain owner and every account picker now show the email and display name instead of a username, across pages, the auth/groups/domain stores and i18n (both locales) (12-07-2026)
- feat(api): the manager account is now identified and logged in by email, not a username. `accounts` becomes auth-only (email NOT NULL UNIQUE, `username`/`name`/`avatar_url` dropped) and every personal attribute moves to a new 1-1 `account_profiles` table (display name, avatar, phone, address, address complement, city, postal code, country, latitude, longitude) via migrations 1783882664757/20000/21000. `GET`/`PATCH /auth/jwt/me` and the account/group/domain endpoints return email + display name, and the JWT payload and `req.user` carry email. A new `GeocodingService` fills latitude/longitude from the city on a profile save (OpenStreetMap Nominatim, best-effort, never blocking). Invitations create an account by email with an optional display name, no username. Paginated list query builders sort/select by entity-property paths so ORDER BY on the joined display name no longer crashes (12-07-2026)
- feat(ui): each group in the groups list (desktop table and mobile card) shows a green check next to its name when the current account is a member of it, read from the session's own memberships. New i18n `groups.memberBadge` in both locales (12-07-2026)
- feat(ui): a new Preferences entry in the personal (user) menu opens a `/preferences` page (available to any authenticated account, like the profile page) gathering the account's device-scoped settings: interface language, appearance (light, dark or system) and the default table page size (the shared `LIST_LIMIT_STORAGE_KEY`, so it applies to every list). The page follows the standard layout (alert header, uppercase section headings and cards), with a new `preferences` i18n namespace in both locales (12-07-2026)
- feat(ui): a re-login now returns the user to the last protected route they were on instead of the dashboard, so a dropped or expired session (or a manual logout) no longer loses their place. The route is kept in localStorage (`manager-last-route`) via a new `useLastRoute` composable; the auth middleware remembers every authenticated non-login, non-public route and the login page and middleware resolve back to it, never to `/login` (no loop), never storing a route forbidden to the current account (12-07-2026)
- feat(ui): the group Members tab gets the standard paginated and searchable list (10/25/50 page size, debounced search, first-load skeleton), kept as a div list rather than a table. The add-member picker becomes a server-side multi-select typeahead: `USelectMenu` in async mode (`v-model:search-term`, `ignore-filter`, `reset-search-term-on-select=false`) fetching only the top 25 non-members per debounced keystroke, whole option objects bound so labels never fall back to raw ids, and the checked accounts added in one bulk call. The assign-all and remove-all counters read the group's `nonMemberCount` and `memberCount`, so they stay correct without ever loading the account list. `ListToolbar` hides its mobile sort select when a page passes no sortable columns (12-07-2026)
- feat(api): the group members list (`GET /groups/:id/members`) is now server-side paginated and searchable (LIKE on the member account's username, name or email), joining group_members to accounts. `GET /accounts/names` gains `notInGroup`, `search` and `limit` so the member picker is a non-member typeahead (results capped 1-50, absent `limit` keeps the full list for the other callers). The group detail carries `nonMemberCount` (a COUNT of assignable accounts) for the assign-all counter. A new `POST /groups/:id/members/bulk` adds several accounts in one call, with the same owner-or-root-or-permitted rule and a single anti-escalation pass, idempotent (12-07-2026)
