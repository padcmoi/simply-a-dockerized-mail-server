import type { Locales } from "../../Locales";

export default {
  subtitle:
    "L'état de la machine qui fait tourner ce serveur mail et de ses deux services, rspamd et Postfix, en direct et sur les sept derniers jours.",
  machine: "Machine",
  live: "En direct",
  connecting: "Connexion…",
  offline: "Hors ligne",
  pause: "Figer les courbes",
  resume: "Reprendre le défilement",
  alertsEnabled: "Vous êtes prévenu quand une mesure passe au rouge. Cliquer pour ne plus l'être. Sans effet sur l'email.",
  alertsDisabled: "Vous n'êtes pas prévenu quand une mesure passe au rouge. Cliquer pour l'être. Sans effet sur l'email.",
  alertsOn: "Vous serez prévenu quand une mesure de la machine passe au rouge.",
  alertsOff: "Vous ne serez plus prévenu des mesures de la machine.",
  sampling: "Mesure…",
  unavailableTitle: "Les mesures de la machine sont indisponibles",
  unavailableHint: "Le flux temps réel est injoignable. Il repart tout seul dès qu'il répond à nouveau.",
  rangeWeek: "7 jours",
  rangeDay: "24 h",
  rangeHour: "1 h",
  rangeMinute: "1 min",
  rangeLoading: "Lecture des mesures enregistrées…",
  rangeEmpty: "Rien n'a encore été enregistré sur cette fenêtre",
  rangeFailed: "Les mesures enregistrées sont illisibles",
  nextRefresh: "Prochain rafraîchissement",
  cpu: "CPU",
  cores: "{n} cœurs vCPU",
  load: "Charge moyenne",
  loadBusy: "Charge soutenue",
  loadSaturated: "Plus de travail que de threads",
  memory: "Mémoire",
  memoryDetail: "sur {total}",
  memoryBusy: "Mémoire tendue",
  memorySaturated: "Mémoire presque épuisée",
  disk: "Disque",
  diskUsed: "occupé",
  diskTotal: "capacité",
  diskUnavailable: "Le disque de la machine est hors de portée depuis ce conteneur",
  network: "Réseau",
  networkIn: "entrant",
  networkOut: "sortant",
  networkUnavailable: "L'interface de la machine est hors de portée depuis ce conteneur",
  fail2ban: "Bans fail2ban",
  clamav: "Signatures antivirus",
  clamavHours: "{n} h",
  clamavBuilt: "construites il y a {n} h",
  clamavOffline: "Moteur injoignable",
  clamavUnavailable: "Les signatures antivirus sont illisibles",
  dmarc: "DMARC",
  dmarcEvaluated: "{n} messages évalués",
  dmarcPass: "Conformes",
  dmarcFail: "Non conformes",
  dmarcNoLive:
    "Pas de vue minute pour DMARC : les évaluations d'OpenDMARC sont relues une fois par minute, la dernière minute serait toujours vide. Choisissez 1 h, 24 h ou 7 jours.",
  fail2banBanned: "{n} adresses bannies",
  fail2banUnavailable: "Fail2ban est hors de portée depuis ce conteneur",
  services: "Services",
  rspamd: "Rspamd",
  rspamdScanned: "{n} messages analysés",
  rspamdUnavailable: "Rspamd est hors de portée depuis ce conteneur",
  postfix: "File Postfix",
  postfixWaiting: "{n} messages en file",
  postfixUnavailable: "Le spool de Postfix est hors de portée depuis ce conteneur",
} satisfies Locales["supervision"];
