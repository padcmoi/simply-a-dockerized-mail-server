import type { Locales } from "../../Locales";

export default {
  alertTitle: "Gestion des accès au panneau d'administration.",
  alertDescription:
    "Les comptes root ont accès total. Les autres comptes sont limités aux permissions accordées par leur groupe assigné.",
  inviteButton: "Inviter un utilisateur",
  backToList: "Retour aux comptes",
  allSessions: {
    label: "Gestion des sessions",
    hint: "Voir les sessions actives et expirées de tous les comptes",
    alertTitle: "Toutes les sessions.",
    alertDescription: "Les sessions de tous les comptes, actives et expirées, regroupées par compte.",
    activeTitle: "Sessions actives",
    activeEmpty: "Aucune session active",
    expiredTitle: "Sessions expirées",
    expiredEmpty: "Aucune session expirée",
    online: "En ligne",
    activeCount: "{count} active(s)",
    expiredCount: "{count} expirée(s)",
    colAccount: "Compte",
    colExpired: "Sessions expirées",
    colLastSeen: "Dernier vu",
    viewDetails: "Voir le détail",
    toast: {
      loadFailed: "Échec du chargement des sessions",
      revoked: "Session révoquée",
      revokedAll: "Toutes les sessions révoquées",
      revokeFailed: "Échec de la révocation de la session",
      purged: "Historique des sessions purgé",
      purgeFailed: "Échec de la purge de l'historique",
    },
    detail: {
      activeTitle: "Sessions actives de {account}",
      expiredTitle: "Sessions expirées de {account}",
      revokeAll: "Tout révoquer",
      activeCrumb: "Actives",
      expiredCrumb: "Expirées",
      purge: "Purger l'historique",
      purgeConfirmTitle: "Purger toutes les sessions expirées de {account} ?",
      purgeConfirmDescription:
        "Supprime définitivement l'historique des sessions expirées et révoquées du compte. Les sessions actives ne sont pas touchées.",
    },
  },
  table: {
    username: "Identifiant",
    name: "Nom",
    email: "Email",
    group: "Groupe",
    noGroup: "Aucun groupe",
    manageGroups: "Gérer les groupes",
    editAccount: "Modifier le compte",
    rootAccess: "Accès complet",
    status: "Statut",
    lastLogin: "Dernière connexion",
  },
  invite: {
    byEmail: "Par email",
    byToken: "Par token",
    emailAlertTitle: "Inviter par email.",
    emailAlertDescription:
      "L'invitation est envoyée depuis le domaine choisi (postmaster{'@'}domaine) pour ne pas finir en spam.",
    emailLabel: "Adresse email",
    domainLabel: "Domaine",
    domainHint: "Le mail d'invitation part de l'adresse postmaster de ce domaine.",
    domainPlaceholder: "Sélectionner un domaine",
    groupsLabel: "Groupes",
    groupsHint: "Optionnel. Le groupe par défaut est assigné automatiquement et n'est pas listé ici.",
    groupsPlaceholder: "Sélectionner un ou plusieurs groupes",
    assignSectionTitle: "Assigner des destinataires et alias",
    assignHint: "Ressources existantes et non assignées du domaine choisi, remises à l'invité à l'acceptation. Assignation uniquement, aucun mot de passe n'est modifié.",
    assignRecipients: "Destinataires",
    assignRecipientsPlaceholder: "Sélectionner des destinataires non assignés",
    assignAliases: "Alias",
    assignAliasesPlaceholder: "Sélectionner des alias non assignés",
    ownerSectionTitle: "Propriété du domaine",
    currentOwnerLabel: "Propriétaire actuel :",
    noOwner: "Aucun propriétaire",
    makeOwnerLabel: "Désigner cet utilisateur comme propriétaire du domaine",
    makeOwnerHint: "Appliqué lorsque l'invitation est acceptée. Un domaine a un seul propriétaire ; le propriétaire existant est remplacé.",
    makeOwnerNoRight: "Vous n'avez pas les droits requis pour désigner un propriétaire de domaine.",
    makeOwnerConfirmTitle: "Changer le propriétaire du domaine ?",
    makeOwnerConfirmDescription:
      "La propriété du domaine changera une fois que l'utilisateur invité aura accepté l'invitation et défini son mot de passe. Le propriétaire existant sera alors remplacé.",
    submit: "Envoyer l'invitation",
    domainsLoadFailed: "Échec du chargement des domaines",
    tokenAlertTitle: "Inviter par token.",
    tokenAlertDescription: "Générer un lien à partager manuellement, sans envoi d'email.",
    tokenSoonTitle: "Bientôt disponible",
    tokenSoonHint: "Les invitations par token ne sont pas encore disponibles.",
    groupPerms: {
      title: "Groupe et permissions du domaine",
      hint: "Un groupe dédié à ce domaine reçoit ces permissions de domaine, et rien d'autre (aucune permission globale). Son nom est imposé, pas choisi.",
      switchLabel: "Utiliser un groupe de domaine dédié",
      groupLabel: "Groupe :",
      exists: "existant",
      willCreate: "sera créé",
      permsLabel: "Permissions pour {domain}",
      noRight: "Il faut la permission groupes avec toutes ses actions (ou root) pour gérer les groupes ici.",
      saved: "Permissions du groupe enregistrées",
      failed: "Échec de l'enregistrement des permissions du groupe",
    },
  },
  overviewPage: {
    alertTitle: "Vue d'ensemble du compte.",
    alertDescription: "Tout ce que ce compte possède sur le serveur mail.",
    activeCount: "{count} actifs",
    quotaLabel: "Quota : {value}",
    ownedDomains: "Domaines possédés",
    ownedRecipients: "Destinataires possédés",
    noDomains: "Ce compte ne possède aucun domaine.",
    noRecipients: "Ce compte ne possède aucun destinataire.",
    stats: { groupsSub: "Appartenances aux groupes", aliasesSub: "Alias possédés" },
    actions: { edit: "Modifier le compte", groups: "Gérer les groupes", recipients: "Destinataires", aliases: "Alias" },
    toast: { loadFailed: "Échec du chargement de la vue d'ensemble" },
  },
  ownership: {
    backToAccount: "Retour au compte",
    recipientsTitle: "Destinataires",
    aliasesTitle: "Alias",
    recipientsAlertTitle: "Destinataires de ce compte.",
    recipientsAlertDescription: "Les destinataires que ce compte possède, et ceux non assignés que vous pouvez lui rattacher.",
    aliasesAlertTitle: "Alias de ce compte.",
    aliasesAlertDescription: "Les alias que ce compte possède, et ceux non assignés que vous pouvez lui rattacher.",
    ownedRecipients: "Destinataires possédés",
    ownedAliases: "Alias possédés",
    assignRecipient: "Rattacher un destinataire",
    assignAlias: "Rattacher un alias",
    searchRecipient: "Sélectionner un destinataire non assigné",
    searchAlias: "Sélectionner un alias non assigné",
    noOwnedRecipients: "Ce compte ne possède aucun destinataire.",
    noOwnedAliases: "Ce compte ne possède aucun alias.",
    attach: "Rattacher",
    detach: "Détacher",
    assigned: "Rattaché au compte",
    unassigned: "Détaché du compte",
    failed: "La modification a échoué",
    allDomains: "Tous les domaines",
    filterDomain: "Filtrer par domaine",
  },
  groupsPage: {
    alertTitle: "Appartenance aux groupes de ce compte.",
    alertDescription: "Assignez ou retirez des groupes ; les permissions se mettent à jour immédiatement.",
    title: "Gérer les groupes ({email})",
    breadcrumb: "Groupes",
    pickPlaceholder: "Choisir un groupe à ajouter",
    add: "Ajouter",
    removeTooltip: "Retirer de ce groupe",
    empty: "Aucun groupe assigné",
  },
  editPage: {
    alertTitle: "Modifier le profil de ce compte.",
    alertDescription: "Email, nom affiché, avatar, téléphone, adresse et statut d'activation.",
    title: "Modifier le compte ({email})",
    breadcrumb: "Modifier",
    nameLabel: "Nom affiché",
    emailLabel: "Email",
    avatarUrlLabel: "URL de l'avatar",
    enabledLabel: "Compte activé",
    enabledHint: "Désactiver un compte lui bloque immédiatement la connexion.",
    save: "Enregistrer",
    toast: {
      saved: "Compte mis à jour",
      saveFailed: "Échec de la mise à jour du compte",
      loadFailed: "Échec du chargement du compte",
    },
  },
  toast: {
    invited: "Invitation envoyée",
    inviteFailed: "Échec de l'invitation",
    deleted: "Compte supprimé",
    deleteFailed: "Échec de la suppression",
    groupUpdated: "Groupe mis à jour",
    groupUpdateFailed: "Échec de la mise à jour du groupe",
    loadFailed: "Échec du chargement",
  },
  confirmDelete: "Supprimer ce compte ?",
  confirmDeleteHint: "Supprime définitivement le compte. Les domaines, destinataires et groupes qu'il possède sont conservés mais perdent leur propriétaire.",
} satisfies Locales["accounts"];
